...
class E2E(torch.nn.Module):
    ...
    def __init__(self, idim, odim, args):
        ...
        # TODO(hirofumi): add args.sampling_probability
...
class AttDot(torch.nn.Module):
    ...
    def forward(self, enc_hs_pad, enc_hs_len, dec_z, att_prev, scaling=1.0):
        ...
        # e = torch.sum(self.pre_compute_enc_h * self.mlp_dec(dec_z).view(batch, 1, self.att_dim),
        #               dim=2)  # utt x frame

        # (utt x frame x att_dim) x (utt x att_dim x 1) --> (utt x frame x 1)
        e = torch.bmm(self.pre_compute_enc_h, self.mlp_dec(dec_z).view(batch, self.att_dim, 1)).squeeze(2)
...
class AttLuong(torch.nn.Module):
    '''Dot or general attention by Luong et al.
    ...
    def __init__(self, eprojs, dunits, att_dim, atype):
        super(AttLuong, self).__init__()

        if atype == 'luong_general':
            self.mlp_enc = torch.nn.Linear(eprojs, att_dim)
    ...
    def forward(self, enc_hs_pad, enc_hs_len, dec_z, att_prev, scaling=1.0):
        ...
        if self.pre_compute_enc_h is None:
            ...
            self.h_length = self.enc_h.size(1)
            # utt x frame x att_dim
            if self.atype == 'luong_general':
                self.pre_compute_enc_h = self.mlp_enc(self.enc_h)
            elif self.atype == 'luong_dot':
                self.pre_compute_enc_h = self.enc_h
        ...
        # e = torch.sum(self.pre_compute_enc_h * dec_z.view(batch, 1, self.att_dim),
        #               dim=2)  # utt x frame

        # (utt x frame x att_dim) x (utt x att_dim x 1) --> (utt x frame x 1)
        e = torch.bmm(self.pre_compute_enc_h, dec_z.view(batch, self.att_dim, 1)).squeeze(2)
    ...
    def forward(self, hs_pad, hlens, ys_pad):
        ...
        for i in six.moves.range(olength):
            ...
            if i > 0 and random.random() < self.sampling_probability:
                ...
                z_out = self.embed(torch.max(self.output(z_all[-1]), dim=1)[1]).detach()
