...
class BPTTUpdater(training.StandardUpdater):
    ...
    def __init__(self, train_iter, model, optimizer, device, gradclip=None, error_weight=None, shuffle=False):
    ...
    def update_core(self):
        ...
        if self.shuffle == True:
            ...
            reverse = 0
        if reverse == 0 or train_iter.iteration_detail < 3:
            loss = 0
            count = 0
            state = None
        ...
        elif reverse == 1 and train_iter.iteration_detail >= 3:
            if self.error_weight is not None:
                assert (self.error_weight < 0), "Error_weight is non negative."
                loss_error = 0
                count_error = 0
...
def train(args):
    ...
    if args.resume:
        ...
        rnnlm_args = get_model_conf(args.resume, None)
        rnn = RNNLM(args.n_vocab, rnnlm_args.layer, rnnlm_args.unit)
    ...
    if args.error_label != '':
        updater = BPTTUpdater({'main':train_iter,'error':error_iter}, model,
                              ...
                              gpu_id, gradclip=args.gradclip, error_weight=args.error_weight, shuffle = args.shuffle)
    else:
