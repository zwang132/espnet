...
if [ ${stage} -le -1 ]; then
    ...
    for x in train dev2010 tst2010 tst2013 tst2014 tst2015 tst2018; do
        local/download_and_untar.sh ${datadir} ${x}
    done
fi
...
if [ ${stage} -le 0 ]; then
    ...
    for x in dev2010 tst2010 tst2013 tst2014 tst2015 tst2018; do
        local/data_prep_eval.sh --lc ${lc} --remove_punctuation ${remove_punctuation} ${datadir} ${x}
    done
fi
...
if [ ${stage} -le 1 ]; then
    ...
    utils/filter_scp.pl data/train_nodev/utt2spk < data/train/text.en > data/train_nodev/text.en
    utils/filter_scp.pl data/train_nodev/utt2spk < data/train/text.de > data/train_nodev/text.de
    utils/filter_scp.pl data/dev/utt2spk < data/train/text.en > data/dev/text.en
    utils/filter_scp.pl data/dev/utt2spk < data/train/text.de > data/dev/text.de

    # Divide into En and De
    for x in train_nodev dev dev2010 tst2010 tst2013 tst2014 tst2015 tst2018; do
...
if [ ${stage} -le 2 ]; then
    ...
    cut -f 2- -d " " data/train_nodev.en/text data/train_nodev.de/text | grep -o -P '&.*?;|@-@' | sort | uniq > ${nlsyms}
    cat ${nlsyms}

    # Share the same dictinary between En and De
    echo "<unk> 1" > ${dict} # <unk> must be 1, 0 will be used for "blank" in CTC
    offset=`cat ${dict} | wc -l`
    cat data/train_nodev.en/text data/train_nodev.de/text | cut -f 2- -d " " | grep -v -e '^\s*$' > data/lang_1spm/input.txt
    ...
    data2json.sh --feat ${feat_tr_dir}/feats.scp --bpecode ${bpemodel}.model \
        data/${train_set} ${dict} > ${feat_tr_dir}/data_${bpemode}${nbpe}_mtl.json
    data2json.sh --feat ${feat_dt_dir}/feats.scp --bpecode ${bpemodel}.model \
        data/${train_dev} ${dict} > ${feat_dt_dir}/data_${bpemode}${nbpe}_mtl.json
    for rtask in ${recog_set} ${eval_set}; do
        feat_recog_dir=${dumpdir}/${rtask}/delta${do_delta}
        data2json.sh --feat ${feat_recog_dir}/feats.scp --bpecode ${bpemodel}.model \
            data/${rtask} ${dict} > ${feat_recog_dir}/data_${bpemode}${nbpe}_mtl.json
    done
    ...
    for x in ${train_set} ${train_dev}; do
        ...
        local/update_json.sh --bpecode ${bpemodel}.model ${feat_dir}/data_${bpemode}${nbpe}_mtl.json ${data_dir} ${dict}
    done
fi
...
# lmexpdir=exp/train_plus_wmt18.de_rnnlm_${backend}_2layer_bs256_${bpemode}${nbpe}
mkdir -p ${lmexpdir}
if [ ${stage} -le 3 ]; then
...
if [ -z ${tag} ]; then
    expdir=exp/st_${train_set}_${backend}_${etype}_e${elayers}_subsample${subsample}_unit${eunits}_proj${eprojs}_d${dlayers}_unit${dunits}_${atype}${adim}_aconvc${aconv_chans}_aconvf${aconv_filts}_${opt}_sampprob${samp_prob}_lsm${lsm_weight}_drop${dropout}_bs${batchsize}_mli${maxlen_in}_mlo${maxlen_out}_${bpemode}${nbpe}
    if ${do_delta}; then
        expdir=${expdir}_delta
    fi
    if [ ${asr} != 0 ]; then
      expdir=${expdir}_${asr_type}asr${asr}
    fi
    if [ ${mt} != 0 ]; then
      expdir=${expdir}_mt${mt}
    fi
    if [ ! -z ${input_feeding} ]; then
        expdir=${expdir}_inputfeed
    fi
    if [ ! -z ${pretrained_asr_model} ]; then
        expdir=${expdir}_asrpt
    fi
    if [ ! -z ${pretrained_mt_model} ]; then
        expdir=${expdir}_mtpt
    fi
...
if [ ${stage} -le 4 ]; then
    ...
    ${cuda_cmd} --gpu ${ngpu} ${expdir}/train.log \
        ...
        --verbose ${verbose} \
        --resume ${resume} \
        --train-json ${feat_tr_dir}/data_${bpemode}${nbpe}_mtl.json \
        --valid-json ${feat_dt_dir}/data_${bpemode}${nbpe}_mtl.json \
        --etype ${etype} \
        --elayers ${elayers} \
        --eunits ${eunits} \
        --eprojs ${eprojs} \
        ...
        --asr-model ${pretrained_asr_model} \
        --mt-model ${pretrained_mt_model} \
...
if [ ${stage} -le 5 ]; then
    ...
    (
        ...
        # split data
        splitjson.py --parts ${nj} ${feat_recog_dir}/data_${bpemode}${nbpe}_mtl.json
        ...
        ${decode_cmd} JOB=1:${nj} ${expdir}/${decode_dir}/log/decode.JOB.log \
            ...
            --ngpu ${ngpu} \
            --backend ${backend} \
            --recog-json ${feat_recog_dir}/split${nj}utt/data_${bpemode}${nbpe}_mtl.JOB.json \
...
# tempolary
# for expdir in 10k_3L 5k_3L 10k_5L 5k_5L; do
# for expdir in 10k_3L_wmt 5k_3L_wmt 10k_5L_wmt 5k_5L_wmt; do
#   for rtask in ${recog_set}; do
#     decode_dir=${rtask}
#     mkdir -p ${expdir}/${decode_dir}
#
#     set=`echo ${rtask} | cut -f -1 -d "."`
#     local/score_bleu_pipe.sh exp/${expdir} ${datadir} ${set}
#   done
# done
